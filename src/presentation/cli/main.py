...
@click.option('--output', '-o', type=click.Path(), help='Output file for normalized data')
...
def detect_anomalies(
    ...
    hours: int,
    critical_only: bool,
    use_case: DetectNetworkAnomaliesUseCase = Provide[Container.detect_network_anomalies_use_case]
):
    ...
    try:
        ...
        headers = ["Timestamp", "Node", "Type", "Severity", "Measurement", "Value", "Deviation"]
...
@click.option('--pattern', type=click.Choice(['hourly', 'daily', 'weekly', 'monthly']), default='daily')
...
def analyze_consumption(
    ...
    pattern: str,
    days: int,
    use_case: AnalyzeConsumptionPatternsUseCase = Provide[Container.analyze_consumption_patterns_use_case]
):
    ...
    try:
        ...
        click.echo(tabulate(table_data, headers=["Period", "Avg Flow"], tablefmt="simple"))
...
def calculate_efficiency(
    ...
    days: int,
    show_leaks: bool,
    use_case: CalculateNetworkEfficiencyUseCase = Provide[Container.calculate_network_efficiency_use_case]
):
    ...
    try:
        ...
        click.echo(f"- Period: {result.period_start.strftime('%Y-%m-%d')} to {result.period_end.strftime('%Y-%m-%d')}")
        ...
        click.echo(f"- Water loss: {result.loss_volume:.1f} m³ ({result.loss_percentage:.1f}%)")
...
@click.option('--include', multiple=True, help='Data types to include (nodes, readings, anomalies)')
@click.pass_context
