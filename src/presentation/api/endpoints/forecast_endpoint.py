...
class ForecastResponse(BaseModel):
    ...
    historical_data: Optional[List[Dict]] = Field(None, description="Historical time series data")
    metrics: Optional[Dict] = Field(None, description="Calculated metrics and statistics")
...
async def get_forecast(
    ...
    try:
        ...
        if include_historical and 'historical' in results and not results['historical'].empty:
            historical_data = []
        ...
        metadata.update({
            ...
            'total_historical_points': len(historical_data) if historical_data else 0
        })
...
async def get_latest_forecast(
    ...
    try:
        ...
        if 'forecast' not in results or results['forecast'].empty:
            raise HTTPException(
                status_code=404,
                detail=f"No forecast found for {district_id}/{metric}"
            )
        ...
        return {
            ...
            'next_value': float(forecast_df.iloc[0]['value']) if len(forecast_df) > 0 else None,
            'generated_at': results.get('metadata', {}).get('generated_at', datetime.utcnow().isoformat())
        }
