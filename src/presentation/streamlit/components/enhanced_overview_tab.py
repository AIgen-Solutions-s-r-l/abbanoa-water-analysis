...
class EnhancedOverviewTab:
    ...
    def _render_geographic_visualization(
        self, time_range: str, selected_nodes: List[str]
    ) -> None:
        """Render geographic network visualization."""
        try:
            from src.presentation.streamlit.components.advanced_visualization_helpers import (
                AdvancedVisualizationHelpers,
            )
            ...
            fig = AdvancedVisualizationHelpers.create_geographic_node_map(
                node_data={}, latest_readings=latest_readings
            )
            ...
            with col3:
                ...
                active_count = len(
                    [r for r in latest_readings.values() if r.get("flow_rate", 0) > 0]
                )
    ...
    def _render_system_status_banner(self) -> None:
        ...
        try:
            ...
            health_percentage = (
                (active_nodes / total_nodes * 100) if total_nodes > 0 else 0
            )
    ...
    def _render_core_kpis(self, time_range: str, selected_nodes: List[str]) -> None:
        ...
        try:
            ...
            with col2:
                total_flow = sum(
                    reading.get("flow_rate", 0) for reading in latest_readings.values()
                )
                st.metric("Total Flow Rate", f"{total_flow:.1f} L/s", delta="Real-time")
            ...
            with col4:
                # Data quality score
                quality_scores = [
                    r.get("quality_score", 1.0) for r in latest_readings.values()
                ]
            ...
            with col5:
                ...
                st.metric("System Uptime", f"{uptime:.2f}%", delta="Target: 99.5%")
        ...
        try:
            ...
            with col1:
                ...
                if latest_readings:
                    ...
                    balance_index = (
                        (1 - (flow_std / flow_mean)) * 100 if flow_mean > 0 else 0
                    )
                else:
                    balance_index = 0

                st.metric(
                    "Network Balance", f"{balance_index:.1f}%", delta="Stability index"
                )
            ...
            with col2:
                ...
                st.metric(
                    "Response Time", f"{response_time:.0f}ms", delta="API latency"
                )
            ...
            with col4:
                ...
                if latest_readings:
                    ...
                    peak_ratio = (
                        (current_total / estimated_peak) * 100
                        if estimated_peak > 0
                        else 0
                    )
                else:
                ...
                st.metric(
                    "Peak Load Ratio", f"{peak_ratio:.1f}%", delta="vs. peak capacity"
                )
            ...
            with col5:
                ...
                st.metric(
                    "Maintenance Alerts", f"{maintenance_alerts}", delta="Active issues"
                )

        except Exception as e:
            st.error(f"Error calculating advanced metrics: {e}")

    def _render_advanced_flow_visualization(
        self, time_range: str, selected_nodes: List[str]
    ) -> None:
    ...
    def _render_system_health_gauges(
        self, time_range: str, selected_nodes: List[str]
    ) -> None:
    ...
    def _render_pressure_distribution(
        self, time_range: str, selected_nodes: List[str]
    ) -> None:
    ...
    def _render_data_quality_matrix(
        self, time_range: str, selected_nodes: List[str]
    ) -> None:
        ...
        try:
            ...
            if latest_readings:
                        ...
                        quality_data.append(
                            {
                                ...
                                "Metric": metric,
                                "Quality": quality
                                + np.random.normal(0, 5),  # Add some variation
                            }
                        )
                ...
                if not df_quality.empty:
                    # Create heatmap
                    pivot_quality = df_quality.pivot(
                        index="Node", columns="Metric", values="Quality"
                    )
    ...
    def _render_operational_insights(
        self, time_range: str, selected_nodes: List[str]
    ) -> None:
