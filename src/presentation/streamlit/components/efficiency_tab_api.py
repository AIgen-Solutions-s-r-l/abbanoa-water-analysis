...
class EfficiencyTab:
    ...
    def render(self, time_range: str) -> None:
        ...
        with col1:
            efficiency = (
                efficiency_data.get("efficiency_percentage", 95.0)
                if efficiency_data
                else 95.0
            )
            st.metric(
        ...
        with col2:
            total_input = (
                efficiency_data.get("total_input_volume", 0) if efficiency_data else 0
            )
            st.metric("Total Input", f"{total_input:,.0f} m³", "Input volume")

        with col3:
            total_output = (
                efficiency_data.get("total_output_volume", 0) if efficiency_data else 0
            )
            st.metric("Total Output", f"{total_output:,.0f} m³", "Output volume")

        with col4:
            loss = total_input - total_output
            loss_percent = (loss / total_input * 100) if total_input > 0 else 0
            st.metric("Water Loss", f"{loss:,.0f} m³", f"{loss_percent:.1f}% of input")
        ...
        # Add target line
        fig.add_hline(
            y=95, line_dash="dash", line_color="red", annotation_text="Target: 95%"
        )
                ...
                fig_zones = go.Figure(
                    data=[
                        go.Bar(
                            ...
                            textposition="auto",
                            marker_color=[
                                "green" if e >= 95 else "orange" for e in efficiencies
                            ],
                        )
                    ]
                )
