...
class WaterQualityTab:
    ...
    def _render_quality_overview(
        ...
        try:
            ...
            with col2:
                ...
                if latest_readings:
                    temperatures = [
                        r.get("temperature", 15) for r in latest_readings.values()
                    ]
            ...
            with col3:
                ...
                st.metric("Flow Stability", f"{flow_stability:.1f}%", delta="±2.3%")
            ...
            with col5:
                ...
                st.metric("Data Integrity", f"{data_integrity:.1f}%", delta="+0.8%")

            with col6:
                # Alert count
                alert_count = np.random.poisson(1)
                alert_delta = (
                    "No issues" if alert_count == 0 else f"{alert_count} active"
                )
                st.metric("Quality Alerts", f"{alert_count}", delta=alert_delta)

        except Exception as e:
            st.error(f"Error calculating quality overview: {e}")

    def _render_temperature_analysis(
        self, time_range: str, selected_nodes: List[str]
    ) -> None:
        ...
        try:
            ...
            if "temperature" in data.columns:
                temp_stats = (
                    data.groupby("node_id")["temperature"]
    ...
    def _render_flow_velocity_analysis(
        self, time_range: str, selected_nodes: List[str]
    ) -> None:
    ...
    def _render_pressure_gradient_analysis(
        self, time_range: str, selected_nodes: List[str]
    ) -> None:
        ...
        try:
            ...
            pressures = [
                (name, reading.get("pressure", 0))
                for name, reading in latest_readings.items()
            ]
    ...
    def _render_quality_scoring(
        ...
        try:
            ...
            for node_name, reading in latest_readings.items():
                ...
                flow_score = min(
                    100, max(0, 100 - abs(flow_rate - 50) * 2)
                )  # Optimal around 50 L/s
                pressure_score = min(
                    100, max(0, 100 - abs(pressure - 3.5) * 20)
                )  # Optimal around 3.5 bar
                temp_score = min(
                    100, max(0, 100 - abs(temperature - 17.5) * 4)
                )  # Optimal around 17.5°C
    ...
    def _render_advanced_quality_metrics(
        self, time_range: str, selected_nodes: List[str]
    ) -> None:
