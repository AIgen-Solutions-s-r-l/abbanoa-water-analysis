...
class KPIDashboardTab:
    ...
    def _render_kpi_grid(self, time_range: str, selected_nodes: List[str]) -> None:
        ...
        try:
            ...
            with col2:
                total_flow = sum(r.get('flow_rate', 0) for r in latest_readings.values())
                st.metric(
                    "🌊 Total Flow",
                    f"{total_flow:.1f} L/s",
                    delta="+2.3 L/s",
    ...
    def _render_performance_scorecard(self, time_range: str, selected_nodes: List[str]) -> None:
        """Render performance scorecard with visual indicators."""
        # Create scorecard data
        scorecard_data = {
            'Category': ['Operational', 'Financial', 'Quality', 'Environmental', 'Safety'],
        ...
        with col2:
            ...
            for _, row in df_scorecard.iterrows():
                ...
                st.write(f"{status} **{row['Category']}**: {row['Score']}% {row['Trend']}")
    
    def _render_financial_metrics(self, time_range: str, selected_nodes: List[str]) -> None:
    ...
    def _render_operational_metrics(self, time_range: str, selected_nodes: List[str]) -> None:
    ...
    def _render_strategic_insights(self, time_range: str, selected_nodes: List[str]) -> None:
    ...
    def _render_trend_analysis(self, time_range: str, selected_nodes: List[str]) -> None:
