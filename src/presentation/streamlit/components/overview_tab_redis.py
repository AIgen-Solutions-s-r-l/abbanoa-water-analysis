...
from src.presentation.streamlit.utils.node_mappings import ALL_NODE_MAPPINGS, get_node_display_name
        ...
        if anomalies:
            ...
            for anomaly in anomalies:
                ...
                if anomaly_type == "low_pressure":
                    ...
                    alert["description"] = f"Pressure dropped to {alert['pressure']:.1f} bar"
                    alert["severity"] = "critical"
                    critical_alerts.append(alert)
                elif anomaly_type == "high_flow":
                    alert["title"] = "Flow Spike Detected"
                    alert["description"] = f"Flow rate spiked to {alert['flow']:.1f} L/s"
                    alert["severity"] = "warning"
                    warning_alerts.append(alert)
                elif anomaly_type == "low_flow":
                    alert["title"] = "Low Flow Detected"
                    alert["description"] = f"Flow rate dropped to {alert['flow']:.1f} L/s"
                    alert["severity"] = "info"
