...
class DataOptimizer:
    ...
    def get_optimization_strategy(
        ...
        if estimated_records:
            if estimated_records > self.strategy.LARGE_DATASET_THRESHOLD:
                strategy["use_sampling"] = True
                strategy["sample_rate"] = min(
                    strategy["sample_rate"], self.strategy.SAMPLE_RATE_LARGE
                )
                strategy["progressive_loading"] = True
                strategy[
                    "explanation"
                ] += f" (large dataset: {estimated_records:,} records)"
                ...
                strategy[
                    "explanation"
                ] += f" (medium dataset: {estimated_records:,} records)"
        ...
        if strategy["use_sampling"] and strategy["sample_rate"] < 1.0:
            ...
            if sample_size > 0:
                ...
                if step > 1:
                    ...
                    logger.info(
                        f"Applied sampling: {len(optimized_data)} records (from {original_count})"
                    )

        # Apply aggregation if needed
        if strategy["use_aggregation"]:
            optimized_data = self._aggregate_data(
                optimized_data, strategy["aggregation_interval"]
            )
            logger.info(
                f"Applied {strategy['aggregation_interval']} aggregation: {len(optimized_data)} records"
            )
    ...
    def _aggregate_data(
        self, data: List[SensorReading], interval: str
    ) -> List[SensorReading]:
    ...
    async def get_data_size_estimate(
        self, node_id: UUID, start_time: datetime, end_time: datetime
    ) -> int:
    ...
    def get_performance_recommendations(
        self, time_range_days: int, estimated_records: int
    ) -> List[str]:
...
def get_optimized_data_for_streamlit(
    sensor_repo: ISensorReadingRepository,
    node_id: UUID,
    start_time: datetime,
    end_time: datetime,
) -> Tuple[List[SensorReading], Dict[str, Any]]:
