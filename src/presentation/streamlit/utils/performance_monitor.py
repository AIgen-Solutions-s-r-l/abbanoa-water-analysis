...
class PerformanceMonitor:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "by_component": df.groupby("component")["execution_time"]
            .agg(["mean", "max", "count"])
            .to_dict(),
            "by_time_range": df.groupby("time_range")["execution_time"]
            .agg(["mean", "max", "count"])
            .to_dict(),
            "slowest_operations": df.nlargest(5, "execution_time")[
                ["component", "time_range", "execution_time", "timestamp"]
            ].to_dict("records"),
        }
    ...
    def render_performance_dashboard(self):
        ...
        if not st.session_state.performance_data:
            st.info(
                "No performance data collected yet. Navigate through different tabs and time ranges to collect data."
            )
            return
        ...
        component_stats = (
            df.groupby("component")["execution_time"]
            .agg(["mean", "max", "count"])
            .reset_index()
        )
        ...
        time_range_stats = (
            df.groupby("time_range")["execution_time"]
            .agg(["mean", "max", "count"])
            .reset_index()
        )
        time_range_stats.columns = [
            "Time Range",
            "Avg Time (s)",
            "Max Time (s)",
            "Count",
        ...
        slowest = df.nlargest(10, "execution_time")[
            ["component", "time_range", "execution_time", "timestamp", "success"]
        ]
