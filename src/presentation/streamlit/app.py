...
class DashboardApp:
    ...
    def render_tabs(self) -> None:
        ...
        if use_api:
            ...
            from src.presentation.streamlit.components.efficiency_tab_api import (
                EfficiencyTab,
            )
            from src.presentation.streamlit.components.overview_tab_api import (
                OverviewTab,
            )
            ...
            from src.presentation.streamlit.components.consumption_tab import (
                ConsumptionTab,
            )
            ...
            from src.presentation.streamlit.components.efficiency_tab import (
                EfficiencyTab,
            )
            from src.presentation.streamlit.components.reports_tab import ReportsTab
            ...
            if st.session_state.get("cache_initialized", False):
                from src.presentation.streamlit.components.overview_tab_redis import (
                    OverviewTab,
                )

                overview_tab = OverviewTab()
            else:
                from src.presentation.streamlit.components.overview_tab import (
                    OverviewTab,
                )
        ...
        with tab2:
            ...
            with st.spinner("Loading enhanced overview..."):
                ...
                @performance_monitor.measure_time(
                    "Enhanced Overview Tab", current_time_range
                )
                def render_enhanced_overview_tab():
                    from src.presentation.streamlit.components.enhanced_overview_tab import (
                        EnhancedOverviewTab,
                    )
                    ...
                    return enhanced_overview.render(
                        ...
                        selected_nodes=st.session_state.get(
                            "selected_nodes", ["All Nodes"]
                        ),
                    )
        ...
        with tab3:
            ...
            with st.spinner("Loading KPI dashboard..."):
                ...
                @performance_monitor.measure_time(
                    "KPI Dashboard Tab", current_time_range
                )
                def render_kpi_dashboard_tab():
                    from src.presentation.streamlit.components.kpi_dashboard_tab import (
                        KPIDashboardTab,
                    )
        ...
        with tab4:
            ...
            with st.spinner("Loading water quality analysis..."):
                ...
                @performance_monitor.measure_time(
                    "Water Quality Tab", current_time_range
                )
                def render_water_quality_tab():
                    from src.presentation.streamlit.components.water_quality_tab import (
                        WaterQualityTab,
                    )
        ...
        with tab10:
            ...
            with st.spinner("Loading classic overview..."):
                ...
                @performance_monitor.measure_time(
                    "Classic Overview Tab", current_time_range
                )
    ...
    def run(self) -> None:
        ...
        if st.session_state.get("using_api", False):
            ...
            if st.session_state.get("cache_initialized", False):
                data_source = (
                    "🟢 Hybrid Architecture Active (Redis + PostgreSQL + BigQuery)"
                )
            else:
