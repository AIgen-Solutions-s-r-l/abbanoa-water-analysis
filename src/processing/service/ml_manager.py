...
class MLModelManager:
            ...
            try:
                ...
                if should_retrain:
                    ...
                    if await self._validate_model(new_model, model_type):
                        ...
                        logger.warning(f"Model {model_type} validation failed, skipping deployment")
    ...
    async def _should_retrain(self, model_type: str) -> bool:
        ...
        recent_performance = await self._get_recent_performance(current_model['model_id'])
        if recent_performance and recent_performance['degradation_factor'] > self.performance_degradation_threshold:
            logger.info(f"Model {model_type} performance degraded by {recent_performance['degradation_factor']:.2f}x, retraining")
            return True
    ...
    async def _train_model(self, model_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            X_train, y_train, X_val, y_val, feature_names = await self._get_training_data(model_type)
    ...
    async def _get_training_data(self, model_type: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, List[str]]:
        """Get training data using hybrid strategy."""
        ...
        # Combine data
        all_data = pd.concat([recent_data, medium_data, historical_data], ignore_index=True)
    ...
    async def _fetch_training_data(
        ...
        job_config.query_parameters = [
            self.bigquery_client.client.query_parameter("start_date", "TIMESTAMP", start_date),
            self.bigquery_client.client.query_parameter("end_date", "TIMESTAMP", end_date),
            self.bigquery_client.client.query_parameter("sample_rate", "FLOAT64", sample_rate),
        ]
        
        return self.bigquery_client.client.query(query, job_config=job_config).to_dataframe()
        
    def _prepare_flow_prediction_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        features = pd.DataFrame({
            ...
            'month': df['month'],
            'flow_pressure_ratio': df['flow_rate'] / (df['pressure'] + 0.1),  # Avoid division by zero
        })
    ...
    def _prepare_anomaly_detection_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        """Prepare features for anomaly detection model."""
        features = pd.DataFrame({
            'flow_rate': df['flow_rate'],
            'pressure': df['pressure'],
            'temperature': df['temperature'],
            'flow_change': df['flow_rate'] - df['prev_flow_rate'].fillna(df['flow_rate']),
            'pressure_change': df['pressure'] - df['prev_pressure'].fillna(df['pressure']),
            'hour_of_day': df['hour_of_day'],
            'day_of_week': df['day_of_week'],
        })
    ...
    def _prepare_efficiency_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        target = 0.95 - (features['flow_variance'] / features['avg_flow'].clip(lower=1)) * 0.1
        
        return features.dropna(), target
        
    async def _train_flow_prediction_model(self, X_train: np.ndarray, y_train: np.ndarray):
    ...
    async def _evaluate_model(
        ...
        if model_type == ModelType.ANOMALY_DETECTION:
            ...
            anomaly_scores = await loop.run_in_executor(None, model.decision_function, X_val)
    ...
    async def _validate_model(self, model_info: Dict[str, Any], model_type: str) -> bool:
        ...
        if model_type == ModelType.ANOMALY_DETECTION:
            ...
            if 'rmse' in new_metrics and 'rmse' in current_metrics:
                ...
                return new_metrics['rmse'] <= current_metrics['rmse'] * 1.1  # Allow 10% tolerance
        ...
        for model_type in [ModelType.FLOW_PREDICTION]:  # Start with flow prediction
            ...
            for node_id in nodes:
                try:
                    ...
                    features = await self._get_prediction_features(node_id, timestamp, model_type)
                    ...
                    logger.error(f"Failed to generate prediction for node {node_id}: {e}")
    ...
    async def evaluate_models(self):
        ...
        for model_type, model_info in self.active_models.items():
            try:
                ...
                if performance['rmse'] > model_info.get('baseline_rmse', 0) * self.performance_degradation_threshold:
                    logger.warning(f"Model {model_type} performance degraded significantly")
    ...
    async def _calculate_model_performance(
        ...
        return {
            ...
            'mape': float(np.mean(np.abs((actuals - predictions) / (actuals + 1e-10))) * 100),
            'r2': float(r2_score(actuals, predictions))
        }
        
    async def _store_performance_metrics(self, model_id: str, metrics: Dict[str, float]):
    ...
    async def _get_recent_performance(self, model_id: str) -> Optional[Dict[str, Any]]:
        ...
        if current and baseline:
            ...
            return {
                ...
                'degradation_factor': current_rmse / baseline_rmse if baseline_rmse > 0 else 1.0
            }
    ...
    async def _detect_data_drift(self, model_type: str) -> bool:
        ...
        if recent_stats and historical_stats:
            ...
            flow_drift = abs(recent_stats['mean_flow'] - historical_stats['mean_flow']) > 2 * historical_stats['std_flow']
            pressure_drift = abs(recent_stats['mean_pressure'] - historical_stats['mean_pressure']) > 2 * historical_stats['std_pressure']
    ...
    async def _create_model_record(self, model_type: str) -> str:
        ...
        async with self.postgres_manager.acquire() as conn:
            model_id = await conn.fetchval("""
                ...
                RETURNING model_id
            """,
            f"{model_type}_model", model_type, version, ModelStatus.CREATED
            )
            
        return str(model_id)
        
    async def _update_model_status(self, model_id: str, status: str, error: Optional[str] = None):
        """Update model status."""
        async with self.postgres_manager.acquire() as conn:
            if error:
                await conn.execute("""
    ...
    async def _update_model_record(
        ...
        async with self.postgres_manager.acquire() as conn:
            await conn.execute("""
                ...
                WHERE model_id = $5
            """,
            status, metrics, model_path, {'features': feature_names}, model_id
            )
