...
class BigQueryMonitoringNodeRepository(IMonitoringNodeRepository):
    ...
    async def get_all(
        ...
        if active_only:
            ...
            query_parameters.append(
                bigquery.ScalarQueryParameter("status", "STRING", NodeStatus.ACTIVE.value)
            )
        ...
        if location:
            query += " AND (location_area = @location OR location_site_name = @location)"
            query_parameters.append(
    ...
    async def update(self, node: MonitoringNode) -> None:
        ...
        query_parameters = [
            bigquery.ScalarQueryParameter(key, "STRING" if isinstance(value, str) else "FLOAT64" if isinstance(value, float) else "TIMESTAMP", value)
