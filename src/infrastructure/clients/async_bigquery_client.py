...
class AsyncBigQueryClient:
    ...
    async def execute_query(
        ...
        try:
            ...
            result_df = await asyncio.wait_for(
                self._execute_query_async(query, parameters), timeout=timeout_ms / 1000
            )
            ...
            raise ForecastServiceException(
                f"BigQuery error: {str(e)}", service="bigquery", original_error=e
            )
    ...
    async def _execute_query_async(
        self, query: str, parameters: Optional[List[ScalarQueryParameter]] = None
    ) -> pd.DataFrame:
        ...
        async def _run_query():
            async with self._get_client() as client:
                ...
                query_job = await loop.run_in_executor(
                    None, client.query, query, job_config
                )
                ...
                df = await loop.run_in_executor(None, results.to_dataframe)
        ...
        try:
            async with self._get_client() as client:
                ...
                table = await loop.run_in_executor(
                    None, client.get_table, full_table_id
                )
