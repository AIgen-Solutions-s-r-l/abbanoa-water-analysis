...
class ForecastCalculationService:
    ...
    async def calculate_forecast(
        ...
        try:
            ...
            return await self._fallback_forecast(
                district_id, metric, horizon, include_history_days
            )
    ...
    async def _generate_forecast(
        self, district_id: str, metric: str, horizon: int, confidence_level: float
    ) -> pd.DataFrame:
    ...
    def _calculate_metrics(
        ...
        percent_change = (
            ((forecast_mean - hist_mean) / hist_mean * 100) if hist_mean != 0 else 0
        )
    ...
    def _enhance_forecast_data(
        ...
        if (
            "lower_bound" not in forecast.columns
            and "standard_error" in forecast.columns
        ):
            # Calculate 80% confidence intervals (z = 1.28)
            forecast["lower_bound"] = forecast["value"] - (
                1.28 * forecast["standard_error"]
            )
            forecast["upper_bound"] = forecast["value"] + (
                1.28 * forecast["standard_error"]
            )
        ...
        if len(historical) > 0:
            ...
            forecast["percent_change"] = (
                (forecast["change_from_last"] / last_value * 100)
                if last_value != 0
                else 0
            )
    ...
    async def _fallback_forecast(
        self, district_id: str, metric: str, horizon: int, include_history_days: int
    ) -> Dict[str, any]:
