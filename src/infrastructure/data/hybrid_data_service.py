...
class HybridDataService:
    ...
    async def _flush_write_buffer(self) -> None:
        ...
        try:
            ...
            await self.postgres_manager.insert_sensor_readings_batch(self.write_buffer)
    ...
    async def get_node_data(
        ...
        """
        cache_key = (
            f"node:{node_id}:data:{start_time.date()}:{end_time.date()}:{interval}"
        )
        ...
        for node_id in node_ids:
            ...
            if latest:
                ...
                if node_id in pg_latest:
                    ...
                    # Cache in Redis
                    self.redis_manager.redis_client.hset(
                        f"node:{node_id}:latest", mapping=pg_latest[node_id]
                    )
    ...
    async def get_system_metrics(self, time_range: str = "24h") -> Dict[str, Any]:
        ...
        self.redis_manager.redis_client.setex(
            cache_key, 300, json.dumps(metrics, default=str)  # 5 minutes
        )
            ...
            if abs(flow_rate - avg_flow) > 3 * std_flow:
                anomaly = {
                    ...
                    "deviation_percentage": abs(
                        (flow_rate - avg_flow) / avg_flow * 100
                    ),
                }
                ...
                self.redis_manager.redis_client.zadd(
                    "anomalies:recent",
                    {
                        json.dumps(anomaly, default=str): reading[
                            "timestamp"
                        ].timestamp()
                    },
                )
    ...
    async def _query_postgres(
        self, node_id: str, start_time: datetime, end_time: datetime, interval: str
    ) -> Optional[pd.DataFrame]:
    ...
    async def _query_bigquery(
        ...
        try:
            ...
            if (
                not hasattr(self.bigquery_client, "client")
                or self.bigquery_client.client is None
            ):
                logger.warning(
                    "BigQuery client not properly initialized, skipping BigQuery query"
                )
                return pd.DataFrame()
    ...
    def _cache_dataframe(self, key: str, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            self.redis_manager.redis_client.setex(key, 3600, data)  # 1 hour TTL
        except Exception as e:
