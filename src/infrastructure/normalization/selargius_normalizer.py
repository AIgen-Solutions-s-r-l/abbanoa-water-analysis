...
class SelargiusDataNormalizer:
    ...
    def _read_csv_file(self, file_path: str) -> Tuple[List[str], List[str], pd.DataFrame]:
    ...
    def _create_monitoring_node(self, node_name: str, node_info: Dict) -> MonitoringNode:
    ...
    def _extract_node_readings(
        ...
        for _, row in df.iterrows():
            timestamp = row.name if isinstance(df.index, pd.DatetimeIndex) else row.get('timestamp')
            ...
            for metric in node_info['metrics']:
                ...
                try:
                    ...
                    elif 'flow' in metric_lower or 'portata' in metric_lower or 'l/s' in unit_lower:
                        flow_rate = FlowRate(float(value))
                    elif 'press' in metric_lower or 'bar' in unit_lower:
                        pressure = Pressure(float(value))
                    elif 'volum' in metric_lower or 'm3' in unit_lower or 'm³' in unit_lower:
    ...
    def _calculate_quality_metrics(
        ...
        total_expected_cells = len(df) * sum(len(n['metrics']) for n in node_structure.values())
        ...
        coverage_percentage = (non_null_count / total_expected_cells * 100) if total_expected_cells > 0 else 0
        
        # Count anomalies (simplified - just count extreme values)
        anomaly_count = 0
        for reading in readings:
            if reading.flow_rate and reading.flow_rate.value > 1000:  # Extreme flow rate
                anomaly_count += 1
