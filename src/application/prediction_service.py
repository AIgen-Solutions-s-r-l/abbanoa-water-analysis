...
class PredictionService:
    ...
    def predict_peak_demand(
        ...
        seasonal_factors = self.seasonal_decomposer.get_seasonal_factors(historical_data)
    ...
    def optimize_energy_cost(
        ...
        off_peak_hours = tariffs.get('off_peak', list(range(0, 8)) + list(range(20, 24)))
        ...
        baseline_schedule = demand_forecast / np.sum(demand_forecast) * np.sum(demand_forecast)
        ...
        if np.sum(optimized_schedule) > 0:
            optimized_schedule = optimized_schedule * (total_demand / np.sum(optimized_schedule))
    ...
    def predict_maintenance(
        ...
        if len(pressure) > 10:
            ...
            if pressure_trend < -0.01:  # Degrading
                ...
                if current_pressure > critical_pressure:
                    days_to_critical = (current_pressure - critical_pressure) / degradation_rate
                else:
                    days_to_critical = 0
        ...
        if not risk_factors:
            ...
            if high_risks > 0:
                risk_score = 'high' if high_risks > 1 else 'critical' if pressure[-1] < 2.5 else 'high'
                days_to_maintenance = min(30, min(r.get('days_to_failure', 30) 
                                                 for r in risk_factors if 'days_to_failure' in r))
    ...
    def predict_water_loss(
        ...
        loss_percentage = np.mean(current_loss / flow_in) * 100 if np.mean(flow_in) > 0 else 0
        ...
        return {
            ...
            'analysis': {
                ...
                'max_loss_m3': float(np.max(current_loss)) if len(current_loss) > 0 else 0,
                'night_flow_anomaly': len(night_flow) > 0 and np.mean(night_flow) > np.percentile(flow_out, 25)
            }
        }
    ...
    def _get_maintenance_recommendations(self, risk_score: str) -> List[str]:
        ...
        if risk_score == 'critical':
            ...
            return [
                "Plan maintenance in next quarter",
