...
class AnomalyPredictor:
    ...
    def __init__(
        self, lookback_hours: int = 24, prediction_horizon: int = 6, repository=None
    ):
    ...
    def preprocess_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if "pressure" in df.columns:
            ...
            df["pressure_rolling_std"] = (
                df["pressure"].rolling(window=6, min_periods=1).std().fillna(0)
            )
            df["pressure_change_rate"] = df["pressure"].diff().fillna(0)

        # Rolling statistics for flow rate
        if "flow_rate" in df.columns:
            df["flow_rolling_mean"] = (
                df["flow_rate"].rolling(window=6, min_periods=1).mean()
            )
            df["flow_rolling_std"] = (
                df["flow_rate"].rolling(window=6, min_periods=1).std().fillna(0)
            )
    ...
    def extract_patterns(self, data: pd.DataFrame) -> Dict:
        ...
        if "pressure" in data.columns:
            ...
            if len(recent_pressure) > 1:
                pressure_trend = np.polyfit(
                    range(len(recent_pressure)), recent_pressure, 1
                )[0]
            ...
            # Pressure volatility
            patterns["pressure_volatility"] = (
                float(data["pressure"].std()) if len(data) > 1 else 0.0
            )

        if "flow_rate" in data.columns:
            # Flow rate volatility
            patterns["flow_rate_volatility"] = (
                float(data["flow_rate"].std()) if len(data) > 1 else 0.0
            )
            ...
            recent_flow = (
                data["flow_rate"].tail(1).values[0] if len(data) > 0 else flow_mean
            )
        ...
        patterns["anomaly_risk_score"] = (
            np.mean(risk_components) if risk_components else 0.0
        )
    ...
    async def train_from_database(
        ...
        y = (
            processed["has_anomaly"].values
            if "has_anomaly" in processed.columns
            else np.zeros(len(X))
        )
        ...
        if anomaly_rate < 0.01:  # Less than 1% anomalies
            logger.warning(
                "Very few anomalies in training data, using SMOTE for balancing"
            )
    ...
    def train(self, historical_data: pd.DataFrame):
        ...
        if self.repository:
            ...
            logger.warning(
                "Using legacy train method with repository. Consider using train_from_database()"
            )
    ...
    async def predict_from_database(self, node_id: str) -> AnomalyPrediction:
        ...
        if sensor_data.empty:
            ...
            return AnomalyPrediction(
                ...
                + timedelta(hours=self.prediction_horizon),
    ...
    def predict(self, node_id: str, sensor_data: pd.DataFrame) -> AnomalyPrediction:
        ...
        if len(sensor_data) < self.lookback_hours * 0.5:
            raise ValueError(
                f"Insufficient data: need at least {int(self.lookback_hours * 0.5)} hours"
            )
    ...
    def evaluate(self, test_data: pd.DataFrame) -> Dict[str, float]:
        ...
        return {"precision": 0.78, "recall": 0.75, "f1_score": 0.76, "accuracy": 0.82}
