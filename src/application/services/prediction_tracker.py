...
class PredictionTracker:
    ...
    async def reconcile_predictions(self) -> ReconciliationResult:
        ...
        min_time = predictions["predicted_timestamp"].min() - timedelta(
            hours=self.reconciliation_window_hours
        )
        max_time = predictions["predicted_timestamp"].max() + timedelta(
            hours=self.reconciliation_window_hours
        )
    ...
    async def calculate_metrics(self, days_back: int = 7) -> PerformanceMetrics:
        ...
        tp = len(
            outcomes[(outcomes["predicted"] == True) & (outcomes["actual"] == True)]
        )
        fp = len(
            outcomes[(outcomes["predicted"] == True) & (outcomes["actual"] == False)]
        )
        tn = len(
            outcomes[(outcomes["predicted"] == False) & (outcomes["actual"] == False)]
        )
        fn = len(
            outcomes[(outcomes["predicted"] == False) & (outcomes["actual"] == True)]
        )
    ...
    async def check_performance_degradation(
        self, threshold_f1: float = 0.7, days_back: int = 7
    ) -> bool:
        ...
        if metrics.f1_score < threshold_f1:
            logger.warning(
                f"Model performance degraded: F1={metrics.f1_score:.2f} < {threshold_f1}"
            )
    ...
    async def process_operator_feedback(self, feedback: Dict) -> Dict:
        ...
        feedback_count = (
            await self.repository.get_feedback_count(days=1)
            if hasattr(self.repository, "get_feedback_count")
            else 0
        )
    ...
    def is_within_window(
        self, prediction_time: datetime, actual_time: datetime
    ) -> bool:
            ...
            for _, pred in predictions.iterrows():
                ...
                for _, anomaly in anomalies.iterrows():
                    if self.is_within_window(
                        pred["predicted_timestamp"], anomaly["timestamp"]
                    ):
                        matched = True
                        break
    ...
    async def save_metrics(
        self, metrics: PerformanceMetrics, model_version: str
    ) -> bool:
