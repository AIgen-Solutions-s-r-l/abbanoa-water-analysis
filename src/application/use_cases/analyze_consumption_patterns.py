...
class AnalyzeConsumptionPatternsUseCase:
    ...
    async def execute(
        ...
        if not readings:
            raise ValueError(f"No readings found for node {node_id} in the specified period")
        ...
        peak_hours, off_peak_hours = self._identify_peak_periods(pattern_data["average_consumption"])
    ...
    def _identify_peak_periods(
        self,
        average_consumption: Dict[str, float]
