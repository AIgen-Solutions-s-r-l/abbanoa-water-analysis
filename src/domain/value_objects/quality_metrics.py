...
class DataQualityMetrics:
    ...
    def _validate(self) -> None:
        ...
        if not 0 <= self.coverage_percentage <= 100:
            raise ValueError(f"Coverage percentage must be between 0 and 100: {self.coverage_percentage}")
        
        if self.missing_values_count < 0:
            raise ValueError(f"Missing values count cannot be negative: {self.missing_values_count}")
    ...
    def quality_score(self) -> float:
        ...
        completeness = (self.total_records - self.missing_values_count) / self.total_records * 100
        anomaly_free = (self.total_records - self.anomaly_count) / self.total_records * 100
        
        score = (
            self.coverage_percentage * 0.7 +
            completeness * 0.2 +
            anomaly_free * 0.1
        )
