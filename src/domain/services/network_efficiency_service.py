...
class NetworkEfficiencyService:
    ...
    def calculate_network_efficiency(
        ...
        if not input_nodes or not output_nodes:
            raise ValueError(
                "Network must have both input and output nodes for efficiency calculation"
            )
    ...
    def _get_nodes_by_type(
        self, network: WaterNetwork, node_types: List[str]
    ) -> List[MonitoringNode]:
    ...
    def _calculate_total_flow(
        self, nodes: List[MonitoringNode], start_time: datetime, end_time: datetime
    ) -> float:
            ...
            for reading in readings:
                if reading.volume:
                    vol_val = (
                        ...
                        if hasattr(reading.volume, "value")
                        else reading.volume
                    )
                    total_flow += vol_val
    ...
    def analyze_node_contribution(
        self, network: WaterNetwork, start_time: datetime, end_time: datetime
        ...
        for node in network.nodes:
            ...
            readings = node.get_readings_in_range(start_time, end_time)
            flow_rates = [
                r.flow_rate.value if hasattr(r.flow_rate, "value") else r.flow_rate
                for r in readings
                if r.flow_rate
            ]
