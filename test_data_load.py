...
async def test_data_load():
    ...
    if not df.empty:
        ...
        print(f"\nYou can now:")
        print(f"1. Run comprehensive tests: poetry run python test_hybrid_architecture.py")
        print(f"2. Check the dashboard at http://localhost:8501")
        print(f"3. Run full ETL: poetry run python -m src.infrastructure.etl.bigquery_to_postgres_etl")
    else:
