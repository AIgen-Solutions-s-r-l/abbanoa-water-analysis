...
class BigQueryPipeline:
    ...
    def upload_data(self, df: pd.DataFrame, table_id: str, write_disposition: str = "WRITE_APPEND"):
    ...
    def run_pipeline(self, input_files: List[str], dry_run: bool = False) -> Dict:
        ...
        for file_path in input_files:
            try:
                ...
                results['status'] = "partial_success" if results['files_processed'] > 0 else "failed"
                
        return results


def generate_bq_commands(project_id: str, dataset_id: str, schema_file: str, data_file: str) -> List[str]:
    ...
    commands = [
        ...
        f"",
        f"# Create table with schema",
        f"bq mk --table \\",
        f"  --schema={schema_file} \\",
        f"  --time_partitioning_field=timestamp \\",
        f"  --time_partitioning_type=DAY \\",
        f"  --clustering_fields=_source_file,timestamp \\",
        ...
        f"  --allow_quoted_newlines \\",
        f"  --max_bad_records=10 \\",
        f"  {project_id}:{dataset_id}.sensor_data \\",
        f"  {data_file}",
        f"",
        f"# Query example",
        ...
        f"  AVG(metric_3) as avg_flow_rate,",
        f"  MAX(metric_3) as max_flow_rate,",
        f"  MIN(metric_3) as min_flow_rate",
...
if __name__ == "__main__":
    ...
    # Find CSV files
    csv_files = ["RAWDATA/REPORT_NODI_SELARGIUS_AGGREGATI_30_MIN_20241113060000_20250331060000.csv"]
