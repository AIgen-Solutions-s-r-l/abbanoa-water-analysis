...
export function LayoutWrapper({ children }: LayoutWrapperProps) {
  ...
  useEffect(() => {
    ...
    const mvpDemoRoutes = ['/', '/enhanced-overview', '/monitoring', '/anomalies', '/weather', '/ml-analytics', '/energy-optimization'];
  ...
  if (!isAuthenticated) {
    ...
    const mvpDemoRoutes = ['/', '/enhanced-overview', '/monitoring', '/anomalies', '/consumption', '/about', '/weather', '/ml-analytics', '/energy-optimization'];
    if (process.env.NODE_ENV === 'development' && mvpDemoRoutes.includes(pathname)) {
      return (
        <LayoutProvider>
          {children}
