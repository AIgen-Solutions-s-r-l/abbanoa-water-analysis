...
export function LayoutWrapper({ children }: LayoutWrapperProps) {
  ...
  useEffect(() => {
    ...
    // TEMPORARY: MVP Demo bypass - allow specific routes without auth but WITH layout
          const mvpDemoRoutes = ['/', '/enhanced-overview', '/monitoring', '/anomalies', '/weather', '/predictions', '/energy-optimization', '/infrastructure-map', '/profile', '/settings', '/admin'];
  ...
  if (!isAuthenticated) {
    ...
    const mvpDemoRoutes = ['/', '/enhanced-overview', '/monitoring', '/anomalies', '/consumption', '/about', '/weather', '/predictions', '/energy-optimization', '/infrastructure-map', '/profile', '/settings', '/admin'];
    if (process.env.NODE_ENV === 'development' && mvpDemoRoutes.includes(pathname)) {
      return (
        <LayoutProvider>
          {children}
