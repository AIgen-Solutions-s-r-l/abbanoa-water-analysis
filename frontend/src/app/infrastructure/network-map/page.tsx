...
export default function NetworkMapPage() {
  ...
  const fetchNetworkData = async () => {
    ...
    try {
      // Fetch topology data
      const topologyRes = await fetch('http://localhost:8000/api/v1/network/topology?include_metrics=true');
      if (!topologyRes.ok) {
      ...
      const hierarchyRes = await fetch('http://localhost:8000/api/v1/network/topology/hierarchy');
      if (!hierarchyRes.ok) {
