...
class ARIMAPlusModelExecutor:
    ...
    def _ensure_dataset_exists(self):
        ...
        try:
            ...
            dataset.description = "Machine learning models for water infrastructure forecasting"
            self.client.create_dataset(dataset)
            logger.info(f"Dataset {dataset_id} created successfully")
    
    def _execute_sql_section(self, sql: str, description: str, timeout: int = 300) -> bool:
    ...
    def _load_sql_notebook(self) -> str:
        ...
        notebook_path = "/home/alessio/Customers/Abbanoa/notebooks/forecast_baseline.sql"
    ...
    def _split_sql_statements(self, sql_section: str) -> List[str]:
        ...
        for line in content.split('\n'):
            if line.strip():
                ...
                if (line.startswith('CREATE ') and current_stmt and 
                    not any(keyword in ' '.join(current_stmt[-5:]) for keyword in ['AS', 'SELECT'])):
        ...
        if success:
            ...
            try:
                ...
                for _, row in result.iterrows():
                    logger.info(f"  {row['district_metric']}: {row['record_count']} records, "
    ...
    def step_2_create_models(self) -> bool:
        ...
        for district, metric in model_configs:
            ...
            if self._execute_sql_section(sql, f"Create model: {model_name}", timeout=1200):
                success_count += 1
            else:
    ...
    def step_3_evaluate_models(self) -> bool:
        ...
        if success:
            ...
            result_sql = f"SELECT * FROM `{self.project_id}.{self.dataset_id}.model_evaluation`"
            try:
                result = self.client.query(result_sql).to_dataframe()
                logger.info("Model evaluation results:")
                for _, row in result.iterrows():
                    mape = row['mean_absolute_percentage_error'] * 100
                    logger.info(f"  {row['model_name']}: MAPE={mape:.1f}%, Assessment={row['mape_assessment']}")
    ...
    def step_4_validate_holdout(self) -> bool:
        ...
        logger.warning("Using simulated holdout period - in practice this would be future data")
        ...
        if not self._execute_sql_section(holdout_sql, "Create holdout dataset", timeout=300):
            return False
        
        # Generate forecasts (simplified version due to complexity of actual ML.FORECAST)
        logger.info("Holdout validation prepared - forecast generation would require model retraining")
        logger.info("In production, this would generate forecasts and calculate MAPE on true holdout data")
    ...
    def step_5_generate_summary(self) -> bool:
        ...
        success = self._execute_sql_section(sql, "Create performance summary", timeout=300)
    ...
    def execute_full_pipeline(self) -> Dict:
        ...
        # Final assessment
        success = self.results.get('performance_summary', {}).get('overall_assessment') != 'NEEDS_IMPROVEMENT'
    ...
    try:
        ...
        if results['success']:
            ...
            if 'performance_summary' in results['results']:
                ...
                print(f"Models passed: {summary['models_passed']}/{summary['total_models']}")
