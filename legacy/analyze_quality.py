...
if missing_data:
    for col, info in sorted(missing_data.items(), key=lambda x: x[1]['percentage'], reverse=True):
        print(f"{col:30} {info['count']:6} valori mancanti ({info['percentage']:5.1f}%)")
else:
...
with open('RAWDATA/REPORT_NODI_SELARGIUS_AGGREGATI_30_MIN_20241113060000_20250331060000.csv', 'r') as f:
...
nodes = {}
for i, (h1, h2) in enumerate(zip(header1[2:], header2[2:]), start=2):  # Skip DATA and ORA
...
improved_quality_score = (total_actual_values / total_expected_values) * 100 if total_expected_values > 0 else 0
...
print("\n\n=== RACCOMANDAZIONI ===")
print(f"1. Il file contiene dati per {len(nodes)} nodi, ma solo {len(active_nodes)} hanno dati")
print(f"2. Su {len(header1)} colonne totali, solo {len(columns_with_data)} contengono dati significativi")
print(f"3. Quality score può migliorare da {metadata['validation']['quality_score']:.1f}% a {improved_quality_score:.1f}%")
print("\nStrategia suggerita:")
...
if columns_to_keep:
    ...
    print(f"✓ Dataset filtrato salvato: normalized_data_filtered.csv ({len(columns_to_keep)} colonne)")
else:
