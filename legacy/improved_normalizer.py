...
class ImprovedWaterDataNormalizer:
    ...
    def analyze_and_normalize(self, file_path: str) -> Dict[str, Tuple[pd.DataFrame, Dict]]:
    ...
    def _create_node_dataset(self, df: pd.DataFrame, node_info: Dict, 
                           node_name: str, file_path: str) -> Tuple[pd.DataFrame, Dict]:
        ...
        column_mapping = {
            m['column_name']: m['clean_name'] 
            for m in node_info['metrics']
        }
        ...
        if columns_to_keep:
            ...
            total_actual = sum(filtered_df[col].notna().sum() for col in columns_to_keep)
    ...
    def _create_filtered_dataset(self, df: pd.DataFrame, node_structure: Dict, 
                                file_path: str) -> Tuple[pd.DataFrame, Dict]:
        ...
        # Quality score
        total_expected = len(filtered_df) * len(columns_to_keep)
        total_actual = sum(filtered_df[col].notna().sum() for col in column_mapping.values())
        quality_score = (total_actual / total_expected) * 100
        
        metadata = {
            'dataset_type': 'filtered_all_sensors',
            'total_columns_original': sum(len(n['metrics']) for n in node_structure.values()),
    ...
    def generate_bigquery_configs(self, results: Dict) -> Dict:
        ...
        for dataset_name, (df, metadata) in results.items():
            ...
            table_config = {
                ...
                "clustering_fields": ["_source_file"] + (["_node"] if '_node' in df.columns else []),
...
if __name__ == "__main__":
    ...
    for dataset_name, (df, metadata) in results.items():
        ...
        if 'metrics_kept' in metadata:
            print(f"  - Metriche mantenute: {metadata['metrics_kept']}/{metadata['metrics_total']}")
