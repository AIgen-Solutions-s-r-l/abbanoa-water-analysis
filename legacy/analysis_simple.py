...
with open('data_info.json', 'w') as f:
    json.dump({k: v if not isinstance(v, tuple) else [str(v[0]), str(v[1])] 
               for k, v in data_info.items()}, f, indent=2)
...
print(f"- Intervallo temporale: {data_info['date_range'][0]} - {data_info['date_range'][1]}")
