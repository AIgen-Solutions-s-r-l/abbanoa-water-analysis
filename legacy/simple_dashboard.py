...
ax3.scatter(df[last_week].index, df[last_week]['L/S'], 
...
print(f"  - Massimo: {last_24h['L/S'].max():.2f} L/S alle {last_24h['L/S'].idxmax().strftime('%H:%M')}")
print(f"  - Minimo: {last_24h['L/S'].min():.2f} L/S alle {last_24h['L/S'].idxmin().strftime('%H:%M')}")
