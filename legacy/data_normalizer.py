...
class WaterDataNormalizer:
    ...
    def analyze_csv_structure(self, file_path: str) -> Dict:
        ...
        units_line = (
            lines[1].split(self.config["csv_separator"]) if len(lines) > 1 else []
        )
    ...
    def create_column_schema(self, headers: List[str], units: List[str]) -> Dict:
        ...
        return {"schema": schema, "column_mapping": column_mapping}
    ...
    def _infer_data_type(self, header: str, unit: str) -> str:
        ...
        if any(
            dt in header_lower for dt in ["data", "date", "ora", "time", "timestamp"]
        ):
        ...
        if "temperatura" in header_lower or "temperature" in header_lower:
            ...
            "pressione" in header_lower
            or "pressure" in header_lower
            or "bar" in unit_lower
        ):
    ...
    def normalize_data(self, file_path: str) -> Tuple[pd.DataFrame, Dict]:
        ...
        schema_info = self.create_column_schema(
            ...
            (
                structure["units"]
                if structure["has_units_row"]
                else [""] * len(structure["headers"])
            ),
        )
    ...
    def _process_datetime_columns(
        self, df: pd.DataFrame, schema_info: Dict
    ) -> pd.DataFrame:
    ...
    def _validate_data(self, df: pd.DataFrame) -> Dict:
        ...
        missing_penalty = sum(
            v["percentage"] for v in validation["missing_values"].values()
        )
    ...
    def _detect_time_gaps(self, timestamps: pd.Series) -> List[Dict]:
        ...
        mode_interval = (
            diffs.mode()[0] if len(diffs.mode()) > 0 else pd.Timedelta(minutes=30)
        )
            ...
            for mapping in metadata["column_mapping"].values():
                if mapping["clean_name"] == col:
                    description = mapping.get(
                        "description", f"{mapping['original_name']} ({mapping['unit']})"
                    )
                    break
