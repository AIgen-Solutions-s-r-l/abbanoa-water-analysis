...
class HybridArchitectureTest:
    ...
    async def test_postgres(self):
        ...
        try:
            from src.infrastructure.database.postgres_manager import (
            ...
            async with postgres.acquire() as conn:
                ...
                self.results["postgres"]["details"][
                    "hypertables"
                ] = f"✅ {len(hypertables)} hypertables found"
                ...
                self.results["postgres"]["details"][
                    "aggregates"
                ] = f"✅ {len(aggregates)} continuous aggregates"
    ...
    async def test_bigquery(self):
        ...
        try:
            ...
            self.results["bigquery"]["details"][
                "recent_data"
            ] = f"📊 {count} readings (last 24h)"
    ...
    async def test_etl_pipeline(self):
        ...
        try:
            ...
            async with postgres.acquire() as conn:
                ...
                if recent_jobs:
                    self.results["etl"]["details"][
                        "recent_jobs"
                    ] = f"✅ {len(recent_jobs)} jobs in last 24h"

                    # Check job success rate
                    success_count = sum(
                        1 for job in recent_jobs if job["status"] == "completed"
                    )
                    ...
                    self.results["etl"]["details"][
                        "recent_jobs"
                    ] = "⚠️ No recent jobs found"

            # Test small sync
            from src.infrastructure.etl.bigquery_to_postgres_etl import (
                BigQueryToPostgresETL,
            )
            ...
            self.results["etl"]["details"][
                "test_sync"
            ] = f"✅ Synced {stats['processed_records']} records"
    ...
    async def test_hybrid_service(self):
        ...
        try:
            from src.infrastructure.data.hybrid_data_service import (
                get_hybrid_data_service,
            )
            ...
            if metrics:
                self.results["hybrid_service"]["details"][
                    "system_metrics"
                ] = "✅ Retrieved system metrics"
                self.results["hybrid_service"]["details"][
                    "active_nodes"
                ] = f"📊 {metrics.get('active_nodes', 0)} active nodes"
            else:
                self.results["hybrid_service"]["details"][
                    "system_metrics"
            ...
            if latest:
                self.results["hybrid_service"]["details"][
                    "latest_readings"
                ] = f"✅ Retrieved {len(latest)} node readings"
            else:
                self.results["hybrid_service"]["details"][
                    "latest_readings"
                ] = "❌ No readings retrieved"
    ...
    async def test_dashboard(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.results["dashboard"]["details"][
                    "health"
            ...
            self.results["dashboard"]["details"][
                "health"
            ] = "⚠️ Dashboard not running (start with 'streamlit run src/presentation/streamlit/app.py')"
        except Exception as e:
            self.results["dashboard"]["status"] = "failed"
            self.results["dashboard"]["details"]["error"] = str(e)

        print(
            f"{'✅' if self.results['dashboard']['status'] == 'passed' else '⚠️'} Dashboard tests completed"
        )
    ...
    def print_summary(self):
        ...
        if self.results["postgres"]["status"] != "passed":
            ...
            print(
                "     Then initialize: docker exec -i abbanoa-postgres psql -U postgres < src/infrastructure/database/postgres_schema.sql"
            )
        if self.results["etl"]["status"] != "passed":
            print(
                "  3. Run initial sync: python -m src.infrastructure.etl.bigquery_to_postgres_etl"
            )
