...
def mock_bigquery_client():
    ...
    async def mock_execute_query(query, parameters=None, timeout_ms=None, use_cache=None):
...
class TestForecastLatency:
    ...
    async def test_cold_run_latency_99th_percentile(
        ...
        forecast_use_case = ForecastConsumption(
            forecast_repository=repo
        )
        ...
        assert avg_latency <= 150.0, f"Average latency ({avg_latency:.2f}ms) should be under 150ms"
    
    @pytest.mark.asyncio
    async def test_warm_run_latency(
        self,
        mock_bigquery_client,
        mock_forecast_data
    ):
        ...
        assert p99_warm <= 150.0, (
            f"Warm 99th percentile ({p99_warm:.2f}ms) should be under 150ms"
        )
        assert avg_warm <= 50.0, (
            f"Warm average ({avg_warm:.2f}ms) should be under 50ms"
        )
