...
class BigQueryBenchmark:
    ...
    def __init__(
        self,
        project_id: str = "abbanoa-464816",
        dataset_id: str = "water_infrastructure",
    ):
    ...
    def run_query_benchmark(
        self, query: str, description: str, expected_time_ms: int, iterations: int = 5
    ) -> Dict:
        ...
        logger.info(
            f"  Results: avg={avg_time:.2f}ms, expected<={expected_time_ms}ms, passed={passed}"
        )
    ...
    def run_all_benchmarks(self) -> Dict:
        ...
        benchmarks["summary"] = {
            ...
            "concurrent_queries_successful": benchmarks["concurrent_queries"][
    ...
    def save_results_to_bigquery(self, results: Dict) -> None:
        ...
        for benchmark_name, result in results.items():
            ...
            flattened_result = {
                ...
                "execution_timestamp": result.get(
                    "timestamp", datetime.utcnow().isoformat()
                ),
                **{
                    k: v for k, v in result.items() if k != "query"
                },  # Exclude query text for storage
            }
...
def main():
    ...
    for name, result in results.items():
        ...
        if "total_execution_time_ms" in result:
            ...
            print(
                f"  All Queries Completed: {result.get('all_queries_completed', 'N/A')}"
            )
    ...
    print(
        f"  Concurrent Queries Successful: {summary['concurrent_queries_successful']}"
    )
