...
class MLDataValidator:
    ...
    def validate_data_quality(self):
        ...
        quality_summary = {
            ...
            'low_quality_percentage': (df['low_quality_count'].sum() / df['total_count'].sum() * 100),
            'high_quality_percentage': (df['high_quality_count'].sum() / df['total_count'].sum() * 100),
            'by_district': df.to_dict('records')
        }
        
        self.validation_results['data_quality'] = quality_summary
        
        print(f"  Overall average quality score: {quality_summary['overall_avg_quality']:.3f}")
        print(f"  High quality readings: {quality_summary['high_quality_percentage']:.1f}%")
        print(f"  Low quality readings: {quality_summary['low_quality_percentage']:.1f}%")
    ...
    def validate_temporal_coverage(self):
        ...
        temporal_summary = {
            ...
            'gap_dates': gaps[['date', 'district_id', 'coverage_percentage']].to_dict('records')[:10]  # First 10
        }
        
        self.validation_results['temporal_coverage'] = temporal_summary
        
        print(f"  Date range: {temporal_summary['date_range']['start']} to {temporal_summary['date_range']['end']}")
        print(f"  Average daily coverage: {temporal_summary['avg_daily_coverage']:.1f}%")
    ...
    def validate_anomaly_detection(self):
        ...
        anomaly_summary = {
            ...
            'anomaly_percentage': (result.total_anomalies / result.total_readings * 100) if result.total_readings > 0 else 0
        }
    ...
    def calculate_ml_readiness_score(self):
        ...
        if 'data_availability' in self.validation_results:
            available_tables = sum(1 for t in self.validation_results['data_availability'].values() if t['exists'])
            score_components['data_availability'] = (available_tables / 3) * 100
        
        # Completeness score
        if 'data_completeness' in self.validation_results:
            avg_completeness = np.mean(list(self.validation_results['data_completeness']['avg_completeness'].values()))
            score_components['completeness'] = avg_completeness
        
        # Quality score
        if 'data_quality' in self.validation_results:
            score_components['quality'] = self.validation_results['data_quality']['overall_avg_quality'] * 100
        
        # Temporal coverage score
        if 'temporal_coverage' in self.validation_results:
            score_components['temporal_coverage'] = self.validation_results['temporal_coverage']['avg_daily_coverage']
        
        # Anomaly rate score (inverse - lower is better)
        if 'anomaly_detection' in self.validation_results:
            anomaly_pct = self.validation_results['anomaly_detection']['anomaly_percentage']
            score_components['anomaly_rate'] = max(0, 100 - (anomaly_pct * 10))  # Penalize high anomaly rates
        ...
        if ml_readiness['improvement_areas']:
            print(f"  Areas for improvement: {', '.join(ml_readiness['improvement_areas'])}")
    ...
    def generate_report(self):
        ...
        report_path = OUTPUT_DIR / f'ml_data_validation_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
        ...
        with open(report_path, 'w') as f:
            ...
            if 'data_quality' in self.validation_results:
                ...
                f.write(f"High Quality Readings: {dq['high_quality_percentage']:.1f}%\n")
                f.write(f"Low Quality Readings: {dq['low_quality_percentage']:.1f}%\n")
                f.write("\n")
            
            # Save full JSON results
            import json
            json_path = OUTPUT_DIR / f'ml_validation_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
