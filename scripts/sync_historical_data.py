...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Sync historical data from BigQuery to PostgreSQL"
    )
    parser.add_argument(
        "--days",
        type=int,
    ...
    try:
        ...
        if stats.get("processed_records", 0) > 0:
            ...
            logger.info(
                "You should now be able to view data for the selected time range in the dashboard."
            )
        else:
            logger.warning("⚠️ No records were processed. Check if:")
            logger.warning("1. BigQuery contains data for the requested time range")
            logger.warning(
                "2. The data might already be synced (use --force to re-sync)"
            )
