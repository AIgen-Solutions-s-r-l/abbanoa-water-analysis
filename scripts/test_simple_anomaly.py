...
async def test_simple_anomaly():
    ...
    try:
        ...
        readings = await sensor_repo.get_by_node_id(
            node_id=node_id, start_time=start_time, end_time=end_time, limit=20
        )
        ...
        from src.domain.services.anomaly_detection_service import (
        ...
        result = await use_case.execute(time_window_hours=1, notify_on_critical=False)
