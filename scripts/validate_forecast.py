...
class ForecastValidator:
            ...
            if response.status_code == 200:
                ...
                if missing_fields:
                    ...
                    if forecast_count == horizon:
                        ...
                        print(
                            f"✅ Forecast successful: {forecast_count} points, {duration_ms:.0f}ms"
                        )

                        # Check data quality
                        if forecast_count > 0:
                            first_point = data["forecast_data"][0]
                            test_result["details"]["sample_value"] = first_point.get(
                                "value"
                            )
                        ...
                        test_result["details"][
                            "error"
                        ] = f"Expected {horizon} points, got {forecast_count}"
                ...
                test_result["details"]["error"] = error_data.get(
                    "detail", response.text
                )
                print(
                    f"❌ API error: {response.status_code} - {test_result['details']['error']}"
                )
    ...
    def test_invalid_inputs(self):
        ...
        test_result = {"test": "invalid_district", "status": "failed", "details": {}}

        try:
            response = requests.get(
                f"{self.api_base_url}/api/v1/forecasts/INVALID/flow_rate", timeout=10
            )
    ...
    def test_model_status(self):
        ...
        test_result = {"test": "model_status", "status": "failed", "details": {}}

        try:
            response = requests.get(
                f"{self.api_base_url}/api/v1/forecasts/models/status", timeout=10
            )
