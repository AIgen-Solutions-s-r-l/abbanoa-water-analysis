...
async def test_fixed_repository():
    ...
    try:
        ...
        readings = await sensor_repo.get_by_node_id(
            node_id=primary_node_id, start_time=start_time, end_time=end_time, limit=10
        )
        ...
        if readings:
            ...
            for i, reading in enumerate(readings[:3]):
                print(
                    f"   [{i+1}] {reading.timestamp}: Flow={reading.flow_rate}, Temp={reading.temperature}, Pressure={reading.pressure}"
                )
    ...
    try:
        ...
        for node_id in node_ids:
            readings = await sensor_repo.get_by_node_id(
                node_id=node_id, start_time=start_time, end_time=end_time, limit=5
            )
    ...
    try:
        ...
        if anomalies:
            ...
            for i, anomaly in enumerate(anomalies[:3]):
                print(
                    f"   [{i+1}] {anomaly.timestamp}: Node={anomaly.node_id}, Flow={anomaly.flow_rate}, Temp={anomaly.temperature}"
                )
    ...
    try:
        ...
        anomaly_results = await anomaly_use_case.execute(
            time_window_hours=24, notify_on_critical=False
        )

        print(
            f"✅ Dashboard integration test passed: {len(anomaly_results)} anomalies detected"
        )
