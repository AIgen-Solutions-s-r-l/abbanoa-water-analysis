...
async def test_data_access():
    ...
    config = BigQueryConfig(
        project_id="abbanoa-464816", dataset_id="water_infrastructure", location="EU"
    )
    ...
    readings = await repo.get_by_node_id(
        node_id=node_id, start_time=start_date, end_time=end_date, limit=10
    )
    ...
    if readings:
        ...
        for reading in readings[:5]:
            print(
                f"  {reading.timestamp}: Flow={reading.flow_rate.value if reading.flow_rate else 'N/A'} L/s, "
