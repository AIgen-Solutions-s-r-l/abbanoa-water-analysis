...
def main():
    ...
    if result3 and 'rows' in result3:
        print("\nNode statistics:")
        print("Node ID         | Latest Reading      | Earliest Reading    | Total | Avg Flow")
        print("-" * 85)
        for row in result3['rows']:
            values = [f['v'] if f['v'] else 'NULL' for f in row['f']]
            print(f"{values[0]:15} | {values[2]:19} | {values[3]:19} | {values[4]:5} | {float(values[5]):.2f}")
    ...
    if result4 and 'rows' in result4:
        ...
        for row in result4['rows']:
            values = [f['v'] if f['v'] else 'NULL' for f in row['f']]
            print(f"{values[0]:20} | {values[1]:14} | {values[2]:5} | {values[3]:5} | {values[4]:4}")
