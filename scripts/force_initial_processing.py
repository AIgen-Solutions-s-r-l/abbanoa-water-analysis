...
async def force_initial_processing():
    ...
    service.last_processed_timestamp = datetime(2024, 1, 1)  # Start from beginning of 2024
    ...
    try:
        ...
        async with postgres_manager.acquire() as conn:
            count = await conn.fetchval("SELECT COUNT(*) FROM water_infrastructure.computed_metrics")
            print(f"Computed metrics created: {count}")
            
            count = await conn.fetchval("SELECT COUNT(*) FROM water_infrastructure.sensor_readings")
