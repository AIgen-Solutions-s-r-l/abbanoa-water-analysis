...
class BackupDataProcessor:
    ...
    def process_data_log(self, file_path: Path, node_id: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            df['node_name'] = NODE_MAPPING.get(node_id, {}).get('name', f'Node {node_id}')
            df['node_type'] = NODE_MAPPING.get(node_id, {}).get('type', 'unknown')
            
            # Extract numeric columns (skip first 3 columns: index, date, time)
            numeric_cols = []
            for col in df.columns[3:]:
                if col not in ['timestamp', 'node_id', 'district_id', 'node_name', 'node_type']:
    ...
    def process_process_data(self, file_path: Path, node_id: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if 'pressure_1' in result_df.columns and 'pressure_2' in result_df.columns:
                result_df['pressure'] = result_df[['pressure_1', 'pressure_2']].mean(axis=1)
    ...
    def process_logger_format(self, file_path: Path, node_id: str) -> Optional[pd.DataFrame]:
    ...
    def identify_metrics(self, df: pd.DataFrame, numeric_cols: List[str]) -> Dict[str, str]:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if 'sensor_readings' in results and not results['sensor_readings'].empty:
