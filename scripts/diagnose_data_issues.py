...
class DataDiagnostics:
        ...
        try:
            ...
            async def test_async_fetch():
                try:
                    ...
                    if readings:
                        sample = readings[0]
                        print(f"    Sample: {sample.timestamp}, Flow: {sample.flow_rate}")
    ...
    def print_summary(self):
        ...
        for test_name, result in self.diagnostics_results.items():
            ...
            elif isinstance(result, dict) and "data_fetch" in result and result["data_fetch"] == 0:
        ...
        if "bigquery_auth" in self.diagnostics_results and "FAILED" in str(self.diagnostics_results["bigquery_auth"]):
            print("  1. Check BigQuery authentication - ensure GOOGLE_APPLICATION_CREDENTIALS is set")
            print("  2. Verify project permissions for project 'abbanoa-464816'")
        
        if "dataset_access" in self.diagnostics_results and "NOT_FOUND" in str(self.diagnostics_results["dataset_access"]):
            print("  3. Dataset 'water_infrastructure' not found - check dataset name")
        
        if any("data_fetch" in str(result) and "0" in str(result) for result in self.diagnostics_results.values()):
            print("  4. No data retrieved - check node mappings and time ranges")
            print("  5. Verify data exists in the expected date range (Nov 2024 - Mar 2025)")
