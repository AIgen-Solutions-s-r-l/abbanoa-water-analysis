...
def test_efficiency_tab():
    ...
    try:
        ...
        print(f"   ✓ Efficiency percentage: {efficiency_data.get('efficiency_percentage', 0):.1f}%")
        print(f"   ✓ Loss percentage: {efficiency_data.get('loss_percentage', 0):.1f}%")
        print(f"   ✓ Average pressure: {efficiency_data.get('average_pressure', 0):.2f} bar")
        print(f"   ✓ Average flow: {efficiency_data.get('average_flow', 0):.2f} m³/h")
        print(f"   ✓ Energy efficiency: {efficiency_data.get('energy_efficiency', 0):.2f} kWh/m³")
        ...
        print(f"   ✓ Efficiency trend: {len(trend_data.get('efficiency_trend', []))} points")
        print(f"   ✓ Water loss trend: {len(trend_data.get('water_loss', []))} points")
        print(f"   ✓ Energy consumption trend: {len(trend_data.get('energy_consumption', []))} points")
        ...
        if efficiency_data.get('total_readings', 0) > 0:
            print(f"🎯 SUCCESS: Found {efficiency_data.get('total_readings', 0)} readings from {efficiency_data.get('active_nodes', 0)} nodes")
            print(f"📊 Overall efficiency: {efficiency_data.get('efficiency_percentage', 0):.1f}%")
        ...
        for node_name in expected_nodes:
            if node_name in node_efficiency:
                ...
                print(f"   {node_name}: {efficiency_val:.1f}% efficiency, {pressure_val:.2f} bar - {status}")
            else:
