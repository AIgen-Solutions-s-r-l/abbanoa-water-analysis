...
class PerformanceAnalyzer:
    ...
    def measure_time(self, func_name: str):
        ...
        def decorator(func):
            async def wrapper(*args, **kwargs):
                ...
                print(
                    f"⏱️  {func_name}: {execution_time:.2f}s {'✅' if success else '❌'}"
                )
    ...
    async def test_single_node_performance(
        self, node_name: str, node_id: UUID, time_range_name: str, delta: timedelta
    ):
        ...
        if success and readings:
            ...
            print(
                f"   🕒 Time range: {start_time.strftime('%Y-%m-%d %H:%M')} to {end_time.strftime('%Y-%m-%d %H:%M')}"
            )
            ...
            if success and result:
                ...
                print(
                    f"   💧 Processed {flow_count} flow readings, {pressure_count} pressure readings"
                )
            else:
    ...
    async def test_all_nodes_performance(self, time_range_name: str, delta: timedelta):
        ...
        for node_name, node_id in self.node_mapping.items():
            result = await self.test_single_node_performance(
                node_name, node_id, time_range_name, delta
            )
        ...
        if successful_results:
            ...
            avg_fetch_time = sum(r["fetch_time"] for r in successful_results) / len(
                successful_results
            )
    ...
    async def run_comprehensive_analysis(self):
        ...
        for time_range_name, delta in self.time_ranges.items():
            results, total_time = await self.test_all_nodes_performance(
                time_range_name, delta
            )
    ...
    def generate_report(self, results: List[Dict[str, Any]]):
        ...
        for time_range, range_results in time_range_stats.items():
            ...
            if successful:
                ...
                if max_time > 15:
                    bottlenecks.append(
                        f"⚠️  {time_range}: Individual node taking {max_time:.2f}s"
                    )
                if total_all_nodes_time > 60:
                    bottlenecks.append(
                        f"⚠️  {time_range}: Total time {total_all_nodes_time:.2f}s exceeds 1 minute"
                    )
                if total_records > 100000:
                    bottlenecks.append(
                        f"⚠️  {time_range}: Large dataset ({total_records:,} records)"
                    )
        ...
        if year_results:
            ...
            if year_successful:
                ...
                if year_total_time > 30:
                    print(
                        f"   ⚠️  CRITICAL: 1-year data takes {year_total_time:.2f}s to load"
                    )
                    print("   🔧 Urgent recommendations:")
                    print(
                        "      - Implement data sampling (e.g., daily averages instead of raw data)"
                    )
