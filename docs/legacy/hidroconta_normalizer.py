    ...
    def normalize_teatinos_data(self, data_folder):
        ...
        for file_path in csv_files:
            ...
            try:
                ...
                if df.empty:
                    metadata_summary['processing_errors'].append(f"Empty data: {file_path}")
                    continue
                ...
                metadata_summary['data_types'][data_type].append(file_metadata['filename'])
        ...
        if 'min_date' in metadata_summary['date_range']:
            metadata_summary['date_range']['min_date'] = metadata_summary['date_range']['min_date'].isoformat()
            metadata_summary['date_range']['max_date'] = metadata_summary['date_range']['max_date'].isoformat()
        ...
        metadata_summary['data_quality'] = {
            ...
            ).days if metadata_summary['date_range'] else 0
        }
        ...
        print(f"   📅 Date range: {metadata_summary['date_range'].get('min_date', 'N/A')} to {metadata_summary['date_range'].get('max_date', 'N/A')}")
