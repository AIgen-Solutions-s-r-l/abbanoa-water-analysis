...
for col, desc in column_mapping.items():
    if any(keyword in desc.upper() for keyword in ['TEMPERATURA', 'PORTATA', 'PRESSIONE']):
...
ax.set_xticklabels([column_mapping.get(col, col)[:30] for col in corr_cols], rotation=45, ha='right')
...
for i in range(len(corr_cols)):
    for j in range(i+1, len(corr_cols)):
        if abs(correlation_matrix.iloc[i, j]) > 0.7:
            high_corr.append((corr_cols[i], corr_cols[j], correlation_matrix.iloc[i, j]))

if high_corr:
    print("\nCorrelazioni elevate trovate (|r| > 0.7):")
    for col1, col2, corr in high_corr[:5]:
        print(f"  {column_mapping.get(col1, col1)[:40]} <-> {column_mapping.get(col2, col2)[:40]}: {corr:.3f}")
...
for col, desc in key_metrics[:2]:
    ...
    plt.plot(df.index, trend, 'r-', linewidth=2, label=f'Trend (MA {window_size} periodi)')
    plt.ylabel('Trend')
    plt.legend()
...
for col, desc in key_metrics[:3]:
    ...
    print(f"  Stazionarietà probabile: {'Sì' if mean_cv < 0.1 and std_cv < 0.2 else 'No'}")
