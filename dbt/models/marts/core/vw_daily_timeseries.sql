...
district_metrics AS (
  ...
  FROM {{ ref('district_metadata') }} d
  CROSS JOIN {{ ref('metric_definitions') }} m
...
-- Gap-filled data with complete date coverage
gap_filled_data AS (
  SELECT 
    ds.date_utc,
    ds.day_of_week,
...
-- Forward fill for operational continuity
forward_filled AS (
  SELECT 
    *,
    
    -- Forward fill critical operational metrics
    CASE 
...
-- Final output with all analytical features
SELECT 
  -- Time dimensions
  date_utc,
  day_of_week,
